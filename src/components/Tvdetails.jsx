...
import { asyncLoadTvDetail, removeTvDetail } from "../store/actions/tvActions";
import {
  Link,
  Outlet,
  useLocation,
...
const Tvdetails = () => {
  ...
  return info ? (
    <div
      ...
      className="w-screen h-[190vh] px-[7%] relative"
    >
      {/* Navigation */}
      ...
      <div className="w-full flex mt-2">
        ...
        <div className="content ml-7 text-white">
          ...
          <div className="flex text-white items-center gap-x-3 mt-1">
            ...
            <h1 className="text-sm">{info.detail.first_air_date}</h1>
            <span className="text-[6px]">
              <i className="ri-circle-fill"></i>
            </span>
            ...
            <h1>Seasons : {info.detail.seasons.length}</h1>
            <span className="text-[6px]">
              <i className="ri-circle-fill"></i>
            </span>
            <h1 className="text-sm">{info.detail.episode_run_time[0]} min</h1>
          </div>
      ...
      <h1 className="text-3xl text-white font-bold ml-3 mb-2">Seasons</h1>
      <div className="w-full h-[45vh] flex overflow-y-hidden mx-2.5">
        {info.detail.seasons.map((season, index) => {
          return (
            <div className="min-w-[30vh]" key={index}>
              <img
                className="shadow-xl w-[25vh] h-[35vh] object-cover rounded-md"
                src={`https://image.tmdb.org/t/p/original/${season.poster_path})`}
                alt=""
              />
              <h1 className="seasons text-[16px] text-center mt-2 text-zinc-300 font-semibold">
                {season.name}
              </h1>
            </div>
          );
      ...
      {/* Recommendation */}
      <hr className="border-gray-400 mb-5 mt-5 h-[2px]" />
